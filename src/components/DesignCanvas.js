...
function Room({ room }) {
  ...
  const wallTexture = useLoader(
    THREE.TextureLoader,
    `/textures/${room.wallTexture || "/textures/woodenwall.jpg"}`,
    (loader) => {
...
function Chair({ item, roomHeight }) {
  ...
  const color = adjustColorBrightness(item.color || "#fff", item.shade || 50);
...
function Table({ item, roomHeight }) {
  ...
  return (
    <group
      position={[item.x || 0, -roomHeight / 2, item.z || 0]}
      scale={scale * 1.5}
    >
...
function Sofa({ item, roomHeight }) {
  const fabricTexture = useLoader(
    THREE.TextureLoader,
    "/textures/fabric.jpeg",
    (loader) => {
      loader.setCrossOrigin("anonymous");
    }
  );
  const scale = item.scale || 1;
  const color = adjustColorBrightness(item.color || "#fff", item.shade || 30);
...
function Lamp({ item, roomHeight }) {
  ...
  const scale = item.scale || 1;
  const color = adjustColorBrightness(item.color || "#fff", item.shade || 70);
...
function Scene({ room, furniture, viewMode, onUpdateFurniture }) {
  ...
  const plane = useRef(
    new THREE.Plane(new THREE.Vector3(0, 1, 0), -room.height / 2)
  );
  ...
  const handleMouseDown = (event) => {
    ...
    if (intersects.length > 0) {
      const index = furniture.findIndex(
        (item) => item.id === intersects[0].object.parent.userData.id
      );
      if (index !== -1) {
        setSelected(index);
      }
  ...
  return (
    <>
      ...
      {furniture.map((item, index) => (
        <group
          key={item.id}
          ref={furnitureRefs.current[index]}
          userData={{ id: item.id }}
        >
          {item.type === "chair" && (
            <Chair item={item} roomHeight={room.height} />
          )}
          {item.type === "table" && (
            <Table item={item} roomHeight={room.height} />
          )}
          {item.type === "sofa" && (
            <Sofa item={item} roomHeight={room.height} />
          )}
          {item.type === "bookshelf" && (
            <Bookshelf item={item} roomHeight={room.height} />
          )}
          {item.type === "tvstand" && (
            <TVStand item={item} roomHeight={room.height} />
          )}
          {item.type === "bed" && <Bed item={item} roomHeight={room.height} />}
          {item.type === "lamp" && (
            <Lamp item={item} roomHeight={room.height} />
          )}
        </group>
      ))}
      <OrbitControls
